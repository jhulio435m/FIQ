...
start_local() {
  ...
  (cd "${LOCAL_DIR}" && ${compose_cmd} -f compose.yaml up -d --build)
}
...
start_remote() {
  ...
  if [[ "${host}" == "${NODE2_HOST}" ]]; then
    ssh -F /dev/null -i "${NODE2_SSH_KEY}" "${host}" "cd ${REMOTE_DIR} && ${compose_cmd} -f compose.yaml up -d --build"
  else
    ssh -F /dev/null "${host}" "cd ${REMOTE_DIR} && ${compose_cmd} -f compose.yaml up -d --build"
  fi
}
...
main() {
  ...
  write_env "${tmp_dir}/node1.env" 1 "${NODE1_NAME}" "${NODE1_IP}"
  write_env "${tmp_dir}/node2.env" 2 "${NODE2_NAME}" "${NODE2_IP}"
  write_env "${tmp_dir}/node3.env" 3 "${NODE3_NAME}" "${NODE3_IP}"

  put_env_remote 1 "${tmp_dir}/node1.env"
  put_env_remote 2 "${tmp_dir}/node2.env"
  install -m 0600 "${tmp_dir}/node3.env" "${LOCAL_DIR}/.env"
